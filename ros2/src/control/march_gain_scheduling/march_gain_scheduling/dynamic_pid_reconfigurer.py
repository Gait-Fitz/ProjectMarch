...
class GaitType(Enum):
    ...
    @classmethod
    def exists(cls, gait_type: str, node: Node) -> bool:
        ...
        return (gait_type is not None and gait_type != "") and (
            gait_type in GaitType._value2member_map_
            or bool(node.get_parameters_by_prefix("gait_types.{gait_type}".format(gait_type=gait_type)))
        )
...
class DynamicPIDReconfigurer:
    ...
    def __init__(self, joint_list: List[str], node: Node):
        ...
        self._srv: Service = self._node.create_service(
            ...
            callback=lambda _, __: Trigger_Response(success=True, message=_configuration),
        )
        ...
        self._publisher: Publisher = self._node.create_publisher(
            msg_type=JointPIDs, topic="/march/dynamic_reconfigure/PIDs", qos_profile=DEFAULT_HISTORY_DEPTH
        )
    ...
    def gait_selection_callback(self, msg: CurrentGait) -> None:
        ...
        if needed_gains != self._current_gains:
            self._node.get_logger().info("Beginning PID interpolation for gait type: {0}".format(self._gait_type))
            ...
            self._timer = self._node.create_timer(
                self.TIME_BETWEEN_GRADUAL_UPDATES, lambda: self.update_params(needed_gains, begin_time, self._linearize)
            )

    def update_params(
        self, needed_gains: List[List[float]], begin_time: Time = None, change_gradually: bool = False
    ) -> None:
    ...
    def stop_update_params_callback(self, begin_time: Time) -> None:
        """To destroys the current timer object and stops the callback when it reached its end.
        ...
        if self._node.destroy_timer(self._timer):
            time_diff: Time = self._node.get_clock().now() - begin_time
            self._node.get_logger().info("PID interpolation finished in {0}".format(time_diff.to_msg()))
        else:
    ...
    def get_needed_gains(self, gait_type: GaitType = None) -> List[List[float]]:
        """Returns all the pid integer values of the gait type.
    ...
    def get_joint_parameter_value(self, joint_name: str, param: str, gait_type: GaitType = GaitType.DEFAULT) -> float:
        """Returns a parameter FLOAT value of the specified joint.
        ...
        """
        param_suffix = "." + joint_name + "." + param
        param_name = "gait_types." + gait_type.value + param_suffix
        default_param_name = "gait_types." + GaitType.DEFAULT.value + param_suffix
