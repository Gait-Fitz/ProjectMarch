"""Author: Eline Troquay, MVII; George Vegelien, MVII."""
import os
import subprocess  # noqa: S404. Subprocess can be unsafe but is necessary in this case.
...
class Headset:
    """Abstract class for the headset data"""
    ...
    def preprocessing(self, data):
        data = data[:, 0:self.number_of_channels]
        data = data.transpose()
...
class Unicorn(Headset):
    ...
    def get_data(self, number_of_samples: int):
        ...
        byte_offset = line_byte_size + (os.stat(self.data_file_loc).st_size % self.BIN_SINGLE_LINE_BYTE_SIZE)
        with open(self.data_file_loc, 'rb') as file:
            file.seek(-byte_offset, os.SEEK_END)
            convert_proces = subprocess.Popen(['unicorn_converter', '-'], stdin=file,
