...
class AccelerationSoftLimitError(Exception):
    def __init__(self, joint_name: str, acceleration: float, limit: float):
        """Class to raise an error when joint trajectory will be outside of
        acceleration soft limits"""
        msg = (
            f"{joint_name} will be outside of acceleration limits, "
            f"acceleration: {acceleration}, acceleration limit: {limit}."
        )

        super(AccelerationSoftLimitError, self).__init__(msg)
